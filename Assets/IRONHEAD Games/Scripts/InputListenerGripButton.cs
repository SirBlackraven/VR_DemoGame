...
public class InputListenerGripButton : MonoBehaviour
{
        ...
        foreach (var device in devices)
        {
            ...
            if (device.isValid)
            {
                ...
                if (device.TryGetFeatureValue(CommonUsages.gripButton, out inputValue) && inputValue)
                {
                    ...
                    {
                        isPressed = true;
                        DebugManagerScript.Instance.AddMessage("Grip OnPress event is called");
                ...
                else if (isPressed)
                {
                    isPressed = false;
                    OnRelease.Invoke();
                    DebugManagerScript.Instance.AddMessage("Grip OnRelease event is called");
